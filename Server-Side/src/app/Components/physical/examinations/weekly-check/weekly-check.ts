...
export class WeeklyCheckComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    try {
      ...
      console.log('✅ Sections loaded in component:', this.allSections.length, this.allSections);
      // Only initialize form after data is loaded
      this.initializeSectionSelectionForm();
    } catch (error) {
      console.error('❌ Error loading sections in component:', error);
      this.allSections = [];
    } finally {
  ...
  initializeSectionSelectionForm(): void {
    if (this.allSections.length === 0) {
      console.warn('⚠️ No sections available to initialize form');
      return;
    }
    ...
    console.log('✅ Form initialized with', Object.keys(controls).length, 'controls');
  }
