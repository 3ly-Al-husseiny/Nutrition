...
export class ExaminationsService {
  private readonly STORAGE_KEY = 'weekly_check_history';
  ...
  private async loadSectionsFromJSON(): Promise<void> {
    try {
      ...
      console.log('✅ Examination sections loaded from JSON:', this.sections.length);
    } catch (error) {
      console.error('❌ Error loading examination sections from JSON:', error);
  ...
  getOverallResult(totalScore: number): string {
    if (totalScore >= 80) {
      return 'Excellent! You are in good physical condition.';
    } else if (totalScore >= 60) {
      return 'Good, but some areas need attention. Consider doing exercises.';
    } else {
      return 'Please consider consulting a doctor for the areas that need attention.';
    }
  }
